...
export default function Paiddetails() {
    const { collectionId } = useParams(); // Extract collectionId from URL params
    ...
    // ... (existing code for formatDate, formatTime, handleOptionSelect)
    ...
    return (
        <>
            ...
            <div className="container mt-4">
                {collectionDetails ? (
                    <div>
                        {/* Display inner details of the collection */}
                        <h2 className="mb-4">Details for collection: {collectionId}</h2>
                        <Link to={`/detail/${collectionId}/adddetail`} className="btn btn-primary mb-3">
                            Add Detail
                        </Link>
                        {/* Display inner details array if it exists */}
                        {collectionDetails.collectionData && collectionDetails.collectionData.length > 0 ? (
                            <div className="row row-cols-1 row-cols-md-3 g-4">
                                {[...collectionDetails.collectionData]
                                    .filter(detail => detail.paid) // Filter details where paid is true
                                    .reverse()
                                    .map((detail, index) => (
                                        <div key={index} className="col">
                                              <div className={`card h-100 ${!detail.paid ? 'border-danger' : 'border-success'}`} style={{ boxShadow: '0px 0px 10px 0px rgba(0,0,0,0.2)', borderRadius: '15px', backgroundColor: '#f8f9fa' }}>
                                    <div className="card-body position-relative">
                                        {/* Display inner detail properties */}
                                            ...
                                            <ul className="dropdown-menu dropdown-menu-end" aria-labelledby={`dropdownMenuButton${index}`}>
                                                <li>
                                                    <button
                                                        ...
                                                        onClick={() => handleOptionSelect(detail._id, 'delete')}
                                                    >
                                                ...
                                                <li>
                                                    <button
                                                        className="dropdown-item"
                                                        onClick={() => handleOptionSelect(detail._id, 'paid')}
                                                    >
                                        ...
                                        <h4 className="card-subtitle mb-2">
                                            Total: ₹ {detail.rate * detail.weight}
                                        </h4>
                                        {detail.paid && (
                                            <div className="tick-mark">&#10003; Paid</div>
                                        )}
                                        {detail.delete && (
                                            <div className="cross-icon">&#10005;</div>
                                        )}
                                        <div className="card-text text-end position-absolute bottom-0 end-0">
                                            <h5 className="card-title">{formatDate(detail.date)}</h5>
                                            <p>
